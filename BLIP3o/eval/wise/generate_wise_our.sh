#!/bin/bash
# Generate WISE images using self-evolving trained model
...
STEPS="${STEPS:-50}"
N_CHUNKS="${N_CHUNKS:-8}"
OUTDIR="${OUTDIR:-${CHECKPOINT_DIR}/wise_images}"

echo "=== WISE Generation (Self-Evolving) ==="
...
for i in $(seq 0 $(($N_CHUNKS - 1))); do
    CUDA_VISIBLE_DEVICES=$i python "${SCRIPT_DIR}/generate_wise_our.py" \
        ...
        --steps "$STEPS" --outdir "$OUTDIR" --index $i --n_chunks $N_CHUNKS &
done
wait
