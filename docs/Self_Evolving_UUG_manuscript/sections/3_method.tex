...
\end{equation}
where $\phi_p$ parameterizes a \textbf{Proposer} $\pi_{\phi_p}(q\mid\mathcal{I})$, $\phi_s$ a \textbf{Solver} $\pi_{\phi_s}(a\mid\mathcal{I},q)$, and $\phi_g$ a \textbf{Generator} $\pi_{\phi_g}(\hat{\mathcal{I}}\mid t)$. We use only model-native operations (QA, captioning, and text-conditioned generation) plus similarity computations from frozen representations; we do not assume access to architecture-specific latents or any external judge.
...
\begin{equation}
    ...
    \label{eq:gen_reward}
\end{equation}
and we update $\phi_g$ using the same KL-regularized policy-gradient recipe as in the visual understanding loop, which makes the optimization procedure consistent across image generation paradigms.

\paragraph{Coupling design.}
The two loops are coupled only through the Solver: the Proposer is trained purely from visual understanding signals, while image generation rewards become stronger as the Solver becomes a more reliable evaluator.
