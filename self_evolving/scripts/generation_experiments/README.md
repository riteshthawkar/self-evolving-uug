...
- generator update: `--generator_update_rule reinforce` (default baseline)
...
Optional DPO variant:
- pass `--generator_update_rule dpo` with `--dpo_beta`, `--dpo_label_smoothing`, and `--dpo_min_reward_gap`.
...
- `logs/dpo_pairs.jsonl` (only for DPO-style runs)
- `iter_log.jsonl`
