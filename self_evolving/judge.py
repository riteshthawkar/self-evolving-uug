...
class FrozenJudge:
    ...
    def __init__(
        ...
        """
        Initialize frozen judge.
        
        Args:
            solver: Solver role to create frozen copy from
            ema_decay: EMA decay coefficient (higher = slower update)
            keep_snapshots: Number of historical snapshots for cross-play
        """
    ...
    def update(self, new_solver):
        ...
        Args:
            new_solver: Current solver with updated weights
        """
        ...
        for p_ema, p_new in zip(
            ...
            new_solver.model.parameters()
        ):
            p_ema.data = (
                self.ema_decay * p_ema.data + 
                (1 - self.ema_decay) * p_new.data
            )
    ...
    def _save_snapshot(self):
        ...
        # Save deep copy
        snapshot = copy.deepcopy(self.solver)
    ...
    def evaluate(
        ...
        Args:
            ...
            use_cross_play: Whether to use multiple judge snapshots
            aggregation: How to aggregate cross-play scores ("min", "median", "mean")
    ...
    def reset_to_reference(self, reference_solver):
        ...
        Use periodically to prevent drift too far from reference.
        
        Args:
            reference_solver: Fresh reference solver to reset to
        """
        self.solver = copy.deepcopy(reference_solver)
...
class CrossPlayEvaluator:
    ...
    to detect and prevent reward hacking. If a generator fools the current
    solver but not historical ones, that's a red flag.
    
    Features:
    - Multiple checkpoint management
    - Divergence detection (current vs historical)
    ...
    def __init__(
        ...
        """
        Initialize cross-play evaluator.
        
        Args:
            snapshots_dir: Directory to save/load snapshots
            max_snapshots: Maximum number of snapshots to keep
            divergence_threshold: Threshold for flagging divergence
        """
    ...
    def add_snapshot(self, solver, step: int, name: Optional[str] = None):
        ...
        Args:
            solver: Solver role to snapshot
        ...
        if hasattr(snapshot['solver'], 'model'):
            for param in snapshot['solver'].model.parameters():
                param.requires_grad = False
    ...
    def load_snapshots(self, solver_template):
        ...
        Args:
            solver_template: Template solver to restore into
        """
        import os
        import glob
        ...
        for path in sorted(glob.glob(os.path.join(self.snapshots_dir, "*.pt"))):
            ...
            # Create solver copy
            solver_copy = copy.deepcopy(solver_template)
            if hasattr(solver_copy, 'model'):
                solver_copy.model.load_state_dict(state['model_state'])
                for param in solver_copy.model.parameters():
                    param.requires_grad = False
    ...
    def evaluate_cross_play(
        ...
        Args:
            ...
            current_solver: Current (training) solver
            n_samples: Samples per verification question
            aggregation: How to aggregate ('min', 'mean', 'median')
            
        Returns:
            Tuple of (aggregated_score, per_solver_scores, divergence_flag)
        """
        scores = []
        ...
        if aggregation == "min":
            ...
            all_scores.sort()
            final_score = all_scores[len(all_scores) // 2] if all_scores else 0.0
        else:
            final_score = min(all_scores) if all_scores else 0.0
        
        # Detect divergence
        # If current solver gives much higher score than historical, that's suspicious
    ...
    def get_divergence_rate(self, last_n: int = 100) -> float:
        """Get the rate of divergence flags in recent evaluations."""
    ...
    def should_reset_judge(self) -> bool:
        """Check if frozen judge should be reset due to high divergence."""
        divergence_rate = self.get_divergence_rate()
        return divergence_rate > 0.3  # Reset if 30%+ evaluations show divergence

    def get_stats(self) -> dict:
        """Get cross-play statistics."""
        return {
