...
class ImagePool:
    ...
    def __init__(self, config: ImagePoolConfig):
        ...
        if split and split not in _KNOWN_SPLITS:
            raise ValueError(f"[ImagePool] Unknown split '{config.split}'. Expected one of: {_KNOWN_SPLITS} or None.")
        ...
        if loaded_from_manifest:
            print(
                f"[ImagePool] Loaded {len(self.paths)} images from manifest under: {root} "
                f"(split={split or 'all'})"
            )
        else:
            print(f"[ImagePool] Found {len(self.paths)} images under: {root} (split={split or 'all'})")
    ...
    def _pick_dirs(self, base_dir: str, include_names: Optional[List[str]]) -> List[Tuple[str, str]]:
    ...
    def _scan_directory(self, split: Optional[str]):
        root = self._root
        include_names = list(self.config.include_subfolders) if self.config.include_subfolders else None
        ...
        # Legacy layout used by dataset builder/rebalancer: <root>/images/<dataset>/image.jpg
        scan_root = os.path.join(root, "images") if os.path.isdir(os.path.join(root, "images")) else root
    ...
    def _infer_split_from_record(self, record: Dict, parts: List[str], split_hint: Optional[str]) -> str:
    ...
    def _load_from_manifest(self, split: Optional[str]) -> bool:
        ...
        with open(manifest_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                parts = rel.split(os.sep)
                record_split = self._infer_split_from_record(record, parts, split_hint=split)
    ...
    def get_image(self, idx: int) -> Tuple[Image.Image, dict]:
        ...
        path = self.paths[idx]
        meta = dict(self.metas[idx]) if idx < len(self.metas) else self._build_meta_from_path(path)
